...
import { allUsersRoute, host, getAllGroup } from "../utils/APIRoutes";
  ...
  useEffect( () => {
    const getCurrentUser = async()=>{
      ...
      if(currentUser.isAvatarImageSet){
        ...
        const group = await  axios.get(`${getAllGroup}/${currentUser._id}`);
        //console.log(group);
        if(group.data.length !== 0) {
          group.data.map(e => {
            socket.current.emit("join-room", e._id);
          })
        }
        setContacts([...data.data,...group.data]);
